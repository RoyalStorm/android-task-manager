...
public interface EventRepository {
    ...
    @GET("/api/v1/events")
    Call<EventResponse> getEventsById(@Query("id") Long[] id, @Header("X-Firebase-Auth") String userToken);

    @DELETE("/api/v1/events/{id}")
    Call<Void> delete(@Path("id") Long id, @Header("X-Firebase-Auth") String userToken);

    @PATCH("/api/v1/events/{id}")
    Call<EventResponse> update(@Path("id") Long id, @Body Event event, @Header("X-Firebase-Auth") String userToken);

    @GET("/api/v1/events/instances")
    Call<EventInstanceResponse> getEventInstancesByInterval(@Query("from") Long from, @Query("to") Long to, @Header("X-Firebase-Auth") String userToken);
}
